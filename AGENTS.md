...
- `main.py`: Tkinter UI, hotkey handling, app state, and dictation flow.
...
- `uv run main.py`: run the app locally.
- `uv run main.py --diag`: run with diagnostic metrics enabled.
- `uv run pytest -q`: run the full test suite quietly.
- `.\start_voz.cmd`: Windows helper launcher (optional).
...
- Keep UI logic in `main.py` and STT/transcription logic in `engine.py`; avoid cross-file coupling.
